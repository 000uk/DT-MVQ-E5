...
class DualDistillationTrainer:
    ...
    def __init__(self, model, teacher, train_loader, optimizer, scheduler, device, config):
        ...
        self.w_scl = config.get('model', {}) \
                           .get('weight', {}) \
                           .get('scl', 1.0)
        self.w_kd = config.get('model', {}) \
                           .get('weight', {}) \
                           .get('kd', 1.0)
        self.mrr_ratio = config.get('valid', {}) \
                               .get('mrr_ratio', 0.5)
    ...
    def train_epoch(self, epoch):        
        ...
        w_scl = 0.1*self.w_scl + (self.w_scl - 0.1*self.w_scl) * cosine_factor
    
        for step, (batch_inputs, labels) in enumerate(tqdm(self.train_loader, desc = f"Epoch: {epoch+1}")):
            batch_inputs = {k: v.to(self.device) for k, v in batch_inputs.items()}
            labels = labels.to(self.device)
    
            genre_vector, content_vector = self.model(**batch_inputs)
            ...
            total_loss = w_scl * loss_scl + self.w_kd * loss_kd
