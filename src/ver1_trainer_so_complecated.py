...
class DualDistillationTrainer:
    ...
    def calc_grad_norm(self, loss, layer):
        """
        장르, 내용 벡터가 head에서 부터 나눠지니까,
        head에서 파라미터 가져와서 Gradiend 계산함!
        """
        # requires_grad=True인 파라미터만 추적
        params = [p for p in layer.attention.parameters() if p.requires_grad]
    ...
    def train_epoch(self, epoch):        
        ...
        # for batch_inputs, labels in tqdm(self.train_loader, desc = f"Epoch: {epoch+1}"):
            ...
            # gradient 계산
            norm_main = self.calc_grad_norm(loss_cont, self.model.genre_head)
            norm_sub = self.calc_grad_norm(loss_kd_combined, self.model.genre_head)
            target_scale = 0.6
